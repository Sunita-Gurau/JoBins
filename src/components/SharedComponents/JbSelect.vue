<template>
  <div :class="[disabled ? 'cursor-not-allowed' : '']">
    <div class="relative">
      <div
        class="relative float-label-input"
        :class="selectTheme === 'gray' ? 'top-0' : 'top-1'"
      >
        <input
          :id="name"
          ref="inputElement"
        ...
        >
          <div>
            <JbIcon :iconName="props.leftIcon" :svgFill="props.svgFillColor" />
          </div>
        </div>
        <label
          ...
          >
            <div v-if="isDropdownVisible">
              <ul
                role="listbox"
                :class="{
                  [`${props.dropDownHeight} jb-scrollbar`]:
              ...
              >
                <slot
                  name="optionList"
                  v-bind="{ handleSelectedOption, getOptionLabel }"
                >
                  <li
                    ...
                    <slot :option="option" name="optionData">
                      <div class="flex w-full justify-between items-center">
                        ...
                        >
                          ...
                          <p
                            v-if="dropdownStyle === 'rightContent'"
                            class="text-sm text-gray-1"
                          >
                            {{
                              capitalizeTheFirstLetter(
                                String(getRightContent(option)),
                              )
                            }}
                          </p>
                        </div>
      ...
      >
        {{
          currentOptionSelected
            ? capitalizeTheFirstLetter(
                (currentOptionSelected as DropdownOption).rightContent!,
              )
            : ''
        }}
      </p>
      <div
        v-if="hasError(props.name) && !disabled"
...
let dropdownOptions = ref<
  string[] | number[] | DropdownOption[] | DropdownOptionLabel[]
...
const filteredDropdownOptions = ref<
  string[] | number[] | DropdownOption[] | DropdownOptionLabel[]
>([]);
...
const hasError = computed(
  () => (id: string) => props.errorMessage && props.errorMessage[id],
);
...
const selectPaddingStyle = computed(() => {
  ...
  return props.selectType === 'normalSelect' && props.leftIcon
    ? 'pl-4'
    : 'pl-4';
});
...
const handleKey = (key: string) => {
  if (key === 'Tab' || key === 'Enter') {
    ...
    if (highlightedIndex.value >= 0) {
      ...
      currentOptionSelected.value = getOptionLabel(
        dropdownOptions.value[highlightedIndex.value],
      );
    }
  }
  ...
  if (key === 'Down' && isDropdownVisible.value) {
    highlightedIndex.value =
      highlightedIndex.value >= dropdownOptions.value.length - 1
        ? 0
        : highlightedIndex.value + 1;
  }
...
const filterArray = (options: any[], searchValue: string) => {
  ...
  if (
    ...
    return options.filter((item) =>
      item.label.toLowerCase().includes(searchValueLower),
    );
  } else {
    return options.filter((item) =>
      String(item).toLowerCase().includes(searchValueLower),
    );
  }
};
